...
for i in ./add/absolute/path/*.gz
do 
  fastqc $i -o ./add/absolute/path/output_dir/ -t 8
done
...
fastp -i /Path/to/_R1.fastq.gz \
 -I /Path/to/_R2.fastq.gz \
 -R /Path/to/output_dir/fastp_report \
 -h /Path/to/output_dir/report.html \
 -o /Path/to/output_dir/241155E_R1_clean.fastq.gz \
 -O /Path/to/output_dir/241155E_R2_clean.fastq.gz \
 -t 6 -q 25
...
for i in /Path/to/cleaned_reads/*.gz
do 
  fastqc $i -o /Path/to/output_dir/ -t 16
done
...
unicycler -1 $short_read1 -2 $short_read2 -l $long_reads -o $output_dir -t 8
```
<details style="background-color: black;">
<summary style="font-size: 28px; color: DarkGreen;"><b>Finished commands</b></summary>
...
unicycler -1 $WORK/genomics/1_short_reads_qc/2_cleaned_reads/241155E_R1_clean.fastq.gz -2 $WORK/genomics/1_short_reads_qc/2_cleaned_reads/241155E_R2_clean.fastq.gz -l $WORK/genomics/2_long_reads_qc/2_cleaned_reads/241155E_cleaned_filtlong.fastq.gz -o $WORK/genomics/3_hybrid_assembly/ -t 8
micromamba deactivate
...
To control the quality of the assembled genome, three different tools will be used.
...
quast.py assembly.fasta --circos -L --conserved-genes-finding --rna-finding\
     --glimmer --use-all-alignments --report-all-metrics -o $output_dir -t 8
```
...
quast.py $WORK/genomics/3_hybrid_assembly/assembly.fasta --circos -L --conserved-genes-finding --rna-finding \
 --glimmer --use-all-alignments --report-all-metrics -o $WORK/genomics/3_hybrid_assembly/quast -t 8
micromamba deactivate
```
</details>
...
checkm lineage_wf $inputdir $output_dir -x fasta --tab_table --file $checkm_out/checkm_results -r -t 8
...
Use **64 GB RAM** for this step as well

```bash
cd $WORK
micromamba activate .micromamba/envs/04_checkm2
checkm2 predict --threads 1 --input $path_to/assembly.fasta --output-directory $output_dir
```
...
checkm2 predict --threads 1 --input $WORK/genomics/3_hybrid_assembly/*.fasta --output-directory $WORK/genomics/3_hybrid_assembly/checkm2 
micromamba deactivate
...
prokka $WORK/genomics/3_hybrid_assembly/assembly.fasta --outdir $WORK/genomics/4_annotated_genome --kingdom Bacteria --addgenes --cpus 8
micromamba deactivate
...
- Run GTDBTK using **64 GB RAM**

```bash
micromamba activate 06_gtdbtk
#run gtdb
gtdbtk classify_wf --cpus 1 --genome_dir $input_fna_files --out_dir $output_dir --extension .fna --skip_ani_screen
```
